...
import CompleterPkg::*; 
//////////////////////////////////////////////////////////////////////////////////
/*CHI-Converter is the module that receives commands from the Scheduler of
the system and based on the command’s information it transfers data from
one memory location to another by using the Coherence Hub Interface proto-
col. To accomplish this operation CHI-Converter has to generate a few Read
and Write transactions for each command.*/
//////////////////////////////////////////////////////////////////////////////////
...
//--------------------------------------------------------------------------  
   ...
   assign ReadReqArbValid = (!(FULLCmndBS & !GaveBSCommand)) & (!SigCommandEmpty & ReqCrd != 0 & FreeReadTxnID != 0 & (SigCommand.Length != ReadReqBytes));
   // Enable valid for CHI-Request transaction 
   assign ReadReqV = (!(FULLCmndBS & !GaveBSCommand)) & (!SigCommandEmpty & ReqCrd != 0 & FreeReadTxnID != 0 & ReadReqArbReady & (SigCommand.Length != ReadReqBytes));
   ...
   // ****************** Write Requester ******************
   // Request chanel from Arbiter
   assign WriteReqArbValid = (!(FULLCmndBS & !GaveBSCommand)) & (!SigCommandEmpty & ReqCrd != 0 & FreeWriteTxnID != 0 & (SigCommand.Length != WriteReqBytes));
   // Enable valid for CHI-Request transaction 
   assign WriteReqV = (!(FULLCmndBS & !GaveBSCommand)) & (!SigCommandEmpty & ReqCrd != 0 & FreeWriteTxnID != 0 & WriteReqArbReady & (SigCommand.Length != WriteReqBytes)) ;
