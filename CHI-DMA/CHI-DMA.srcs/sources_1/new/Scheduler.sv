...
module Scheduler#(
  ...
  parameter Chunk             = 5   // number of CHI-Words 
    ...
    reg                                   RegWESig          ;  // WE for Reg that keeps the Addr pointer of Desc
    
    wire       [BRAM_COL_WIDTH    -1 : 0] SigWordLength     ; // Signal to update SentBytes field of Descriptor and Length of Command
    reg                                   WEControl         ; // Signal that FSM provides and controls WE of BRAM 
    
    reg        [BRAM_COL_WIDTH    -1 : 0] NextReadAddr      ; // SrcAddr + SentBytes of Descriptor
    
    assign NextReadAddr = DescDataIn.SrcAddr + DescDataIn.SentBytes ;
    // number of bytes that will be scheduled from Descriptor (which is the min of the remaining bytes and the number of bytes until the aligned Addr of SrcAddr+SentBytes+CHI_DATA_WIDTH*Chunk)
    assign SigWordLength = ((DescDataIn.BytesToSend - DescDataIn.SentBytes < (CHI_DATA_WIDTH * Chunk - NextReadAddr[$clog2(CHI_DATA_WIDTH) - 1 : 0])) ? DescDataIn.BytesToSend - DescDataIn.SentBytes : (CHI_DATA_WIDTH * Chunk - NextReadAddr[$clog2(CHI_DATA_WIDTH) - 1 : 0]));                                                                                  ;
    ...
    assign WE = WEControl ? ((SigWordLength == 0 | (DescDataIn.BytesToSend < DescDataIn.SentBytes)) ? ('b1 << `StatusRegIndx) : ('b1 << `SBRegIndx) ): 0 ;
   ...
   always_comb begin : next_state_logic
     case(state)
       ...
       IssueState :
         begin
           if((DescDataIn.BytesToSend == DescDataOut.SentBytes & !CmdFIFOFULL) | !ReadyBRAM | (DescDataIn.BytesToSend < DescDataIn.SentBytes)) // All Bytes of a Descriptor have been scheduled
             next_state = IdleState ;
       ...
       IssueState : 
         begin         // Schedule a new transaction when posible
           if(CmdFIFOFULL | !ReadyBRAM)begin  // If comand FIFO is FULL or there is not control of BRAM do nothing (Dont schedule)
             ...
             ValidFIFO  = 0                                                                    ;
             RegWESig   = 1                                                                    ;
             IssueValid = SigWordLength != 0 & (DescDataIn.BytesToSend > DescDataIn.SentBytes) ;
             ValidBRAM  = 1                                                                    ;
           end
         end
       WriteBackState : 
